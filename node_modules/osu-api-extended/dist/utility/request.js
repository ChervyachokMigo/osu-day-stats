 ...
 * @returns {Promise<any>} The response
 */
const download = (url, dest, { headers, data, params } = {}, callback) => new Promise(async (res, rej) => {
    ...
    const req = https_1.default.request(url + (params ? '?' + o(params) : ''), { method: 'GET', headers }, response => {
        ...
        if (location) {
            const redirect = (0, exports.download)(location, dest, { headers, data, params }, callback);
            return res(redirect);
        }
        ;
